...
class CC_DATABASE_INTERFACE():
    ...
    def getAssetData(self,asset,_from,_to):
        req = "SELECT date,close,open,mcap FROM asset_price WHERE name = %s AND date >= %s AND date <= %s"
        data = self.exec_n_fetch(req,(asset,_from,_to))
        data_formatted = []
        for date,closePrice,openPrice,mcap in data:
            data_formatted.append({"date":date.isoformat(),"open":openPrice,"close":closePrice,"mcap":mcap})
        return data_formatted
